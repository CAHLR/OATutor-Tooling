...
- **`CAHLR/OATutor` main repo is dangerous to hand-edit:** its content-staging cron does `rm -rf` + regenerates params every 15 min, collapsing experimental back to a copy of default. Fix is a one-line workflow patch (branch `experimental-bktparams-from-tooling`) so the experimental slot copies from this repo's committed output instead — **must merge after** the Tooling PR carrying `bkt/output/experimentalBKTParams.json`.
