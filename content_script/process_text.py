...
def preprocess_text_to_latex(text, tutoring=False, stepMC=False, render_latex="TRUE", verbosity=False):
    ...
    if render_latex:
        ...
        text = re.sub(r"\([\s]*([-\d]+)[\s]*,[\s]*([-\d]+)[\s]*\)", r"(\g<1>,\g<2>)", text) #To account for coordinates
        text = re.sub(r"\s\\\\\"\s", " ", text) #To account for quoted LaTeX expressions.
        text = re.sub(r"\\\\pipe", "|", text) #To account for literal | in mc answers
        ...
        text = re.sub(r"_\(([^)]+)\)", r"_\g<1>", text) # To account for subscript in form of A_(BC) (chage to A_BC)
    ...
    for i in list(range(len(words))):
        ...
        if use_latex(word, render_latex, stepMC):
            if not re.findall(r"[\[|\(][\+\-\*/\(\)\d\s\w]+,[\+\-\*/\(\)\d\s\w]+[\)|\]]", word): # only add in space if is not coordinate
    ...
    for part in parts:
        if any([op in part for op in supported_operators]) or any([op in part for op in supported_word_operators]) and 'info' not in part:
            ...
            if part == "inf" or not part.isalpha():
                return True
        if re.match(r"[\d\.]*[bdhmnprtxyz][.\?\!,\%\$]{,1}$", part) and "y-coord" not in word:
...
def handle_word(word, coord=True):
    ...
    if "log{" in word:
        nums = [a.group(1) for a in re.finditer(r'log\{[^}]+\}\{([^}]+)\}', word)]
        latex_nums = [re.sub(r'\\', r'\\\\', handle_word(n)) for n in nums]
        while re.search(r'log\{', word):
            word = re.sub(r'log\{([^}]+)\}\{[^}]+\}', r'\\log_{\g<1>}\\left(' + latex_nums[0] + r'\\right)', word, count=1)
            latex_nums.pop(0)
        return word
    ...
    coordinates = re.findall(r"(?<!sqrt)[\(|\[][(sqrt)\+\-\*/\(\)_\d\s\w]+,[(sqrt)\+\-\*/\(\)_\d\s\w]+[\)|\]]", word)
    nth_root = re.search(r"sqrt\([+\-\*/\(\)\d\w]+(,)[+\-\*/\(\)\d\w]+\)", word)
    if nth_root:
    ...
    if coord and coordinates and not nth_root:
        ...
        if word[-1] != ')' and word[-1] != ']':
            trailing = word[-1]
            word = word[:-1]
        first = re.search(r'(\(|\[)([-\d\s\D]+),', word)
        rest = word[word.index(first.group(0)) + len(first.group(0)) - 1:]
        second = re.search(r',([-\d\s\D]+)(\)|\])', rest)
        ...
        return re.sub(r"[\(|\[][-\d\D]+,[-\d\D]+[\)|\]]", new_coord, word)
    
    word = re.sub(r"\+/-", "~", word)
    word = re.sub(r"(.+)~", r"\g<1>+plusminus+", word)

    
    scientific_notation = re.findall(r"\(?([\d]{2,})\)?\*([\d]{2,})\*\*", word)
    word = re.sub(":sqrt", ": sqrt", word)
    square_roots = re.findall(r"sqrt\(([^,]*)\,([^\)]*)\)", word)
    word = re.sub(",", "", word)
    for root in square_roots:
        word = re.sub(r"sqrt\("+re.escape(root[0])+re.escape(root[1])+r"\)", r"sqrt("+root[0]+","+root[1]+")", word)
    word = re.sub(r"(^ln[\w])(\(+[\w])", r"\g<1>*\g<2>", word)
    word = re.sub(r"(\)+)([\w])", r"\g<1>*\g<2>", word)
    word = re.sub(r"(\))(\()", r"\g<1>*\g<2>", word)
    word = re.sub(r"([0-9]+)([a-zA-Z])", r"\g<1>*\g<2>", word)
    word = re.sub(r"sqrt\*", r"sqrt", word)
    word = re.sub(r"abs\*", r"abs", word)
    word = re.sub(r'\*\*\(\-0\.', '**(zero', word)
    word = re.sub(r'\*\*\(\-\.', '**(zero', word)
    word = re.sub(r'(?<!(abs)|(log)|(qrt))\(\-', 'negneg(', word)
    word = re.sub(r'\\=', '=', word)
    ...
    bracketsub = re.search(r"\[([^\(^\)]+)\]", word)
    word = re.sub(r"\[([^\(^\)^\[^\]]+)\]", "bracketsub", word)
    # to handle -(.....) missing parenthesis instance
    while re.search(r"-\([^\w\d\(]", word):
        open_par_miss = re.search(r"-\([^\w\d]", word).start() + 1
    ...
    word = re.sub(r"%", "\\\\%", word)
    word = re.sub(r'leftt\+', '\\\\left(', word)
    word = re.sub(r'\+rightt', '\\\\right)', word)
    ...
    for trig in trig_operators:
        ...
        for mat in matches:
            ...
            if word[power_idx] == '{':
                ...
                power = re.match(r'[0-9\.]+', word[power_idx:]).group(0)
            else:
...
def handle_single_limit(lim):
    a = re.search(r"/lim{([^,]+),([^,]+),(.+)}", lim).group(1)
    b = re.search(r"/lim{([^,]+),([^,]+),(.+)}", lim).group(2)
    c = re.search(r"/lim{([^,]+),([^,]+),(.+)}", lim).group(3)
...
def handle_single_integral(integral):
    definite = re.search(r"/int{(.+),([^,]+),([^,]+),([^,]+)}", integral)
    if definite:
        ...
        indefinite = re.search(r"/int{(.+),([^,]+)}", integral)
